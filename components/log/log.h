...
/* WARN: 警告，不影响运行但需注意 */
#define LOG_W(tag, fmt, ...) Log_Printf(LOG_LEVEL_WARN, __FILE__, __LINE__, tag, fmt, ##__VA_ARGS__)

/* INFO: 关键流程信息 */
#define LOG_I(tag, fmt, ...) Log_Printf(LOG_LEVEL_INFO, __FILE__, __LINE__, tag, fmt, ##__VA_ARGS__)
