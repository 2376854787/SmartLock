...
/* 常用分类判断：上层用它们可保持兼容（即使模块ID不同） */
 CORE_INLINE bool ret_is_timeout(ret_code_t r)     { return ret_is_errno(r, RET_ERRNO_TIMEOUT); }
 CORE_INLINE bool ret_is_busy(ret_code_t r)        { return ret_is_errno(r, RET_ERRNO_BUSY); }
 CORE_INLINE bool ret_is_no_mem(ret_code_t r)      { return ret_is_errno(r, RET_ERRNO_NO_MEM); }
 CORE_INLINE bool ret_is_invalid_arg(ret_code_t r) { return ret_is_errno(r, RET_ERRNO_INVALID_ARG); }
