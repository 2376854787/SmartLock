...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    KEY_Init(&key1);
    KEY_Init(&key2);
  LOG_I("测试", "你好我的朋友:%s", "STM32");
  LOG_I("MAIN", "System Initialization Started...");
  HAL_Delay(500);

  LOG_W("SENSOR", "Temp sensor response slow, check wiring!");

  LOG_E("MEM", "Malloc failed at address 0x20001000");

  int count = 99;
  LOG_D("APP", "Current count variable = %d", count);
